...
namespace DogAPI.Areas.Admin.Data
{
    public class CommonFunction
    {
        ...
        public UploadModel VideoUpload(UploadModel model, string videoTitle, string videoDescription)
        {
                ...
                if (file != null)
                {
                    ...
                    if (supportedVideoTypes.Contains(fileExt))
                    {
                        ...
                        if (file.ContentLength < FileSize)
                        {
                            ...
                            Task<string> task = Task.Run(async () => await RunMethod(file, videoTitle, videoDescription));
                            task.Wait();
        ...
        public async Task<string> RunMethod(HttpPostedFileBase VideoUrl,string videoTitle,string videoDescription)
        {
            try
            {
        ...
        {
            UploadInPlayListByvideoId(video.Id, video.Snippet.Title, video.Snippet.Description).Wait();
            //ViewBag.Message = "successfully uploaded." + video.Id;
            //Console.WriteLine("Video id '{0}' was successfully uploaded.", video.Id);
        }

        private async Task UploadInPlayListByvideoId(string VideoId, string Title, string Description)
        {
            //var TagDetails = "";
            //if(tags.Count>0)
            //{
        ...
        public string VideoUpdateById(string VideoId, string Title, string Description, IList<string> tags)
        {
            ...
            Task task = Task.Run(async () => await UploadInPlayListByvideoId(VideoId, Title, Description));
            task.Wait();
